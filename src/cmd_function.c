...
void GetAppCmd()
{
	if (CheckFlag())
	{
		...
		switch (usCmd)
		{
			...
			case CMD_GET_RUNFILE_BEGINPOS_FLAG:
			{
				int iAxis;

				memcpy(&iAxis, pData, 4);
				CmdGetToApp(usCmd, sizeof(bool));
				memcpy((void *)IO_ADDR_BRAM_OUT_DATA, &g_bBeginPosFlag[iAxis], sizeof(bool));
				SetFlagOutOne();
				break;
			}
			...
			case CMD_SET_RUNFILE_CMD:
			{
				...
				memcpy(&CmdBuf, pData + 4, sizeof(FILE_CMD));
